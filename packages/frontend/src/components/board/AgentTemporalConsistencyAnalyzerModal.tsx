import type { AgentTemporalConsistencyAnalyzerReport } from '../../api/mutations.js';

interface Props {
  result: AgentTemporalConsistencyAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentTemporalConsistencyAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing temporal consistency...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No temporal consistency data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-stone-900/20 border border-stone-500/30 rounded-lg px-4 py-3">
                  <p className="text-stone-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Consistency Score</p>
                  <p className="text-stone-200 text-2xl font-bold">{result.fleetAvgConsistencyScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Unstable Agents</p>
                  <p className="text-red-200 text-xl font-bold flex items-center gap-2">
                    {result.unstableAgents}
                    {result.unstableAgents > 0 && (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Short-Term%</th>
                      <th className="px-4 py-3 text-center">Long-Term%</th>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {result.metrics.map((m, i) => (
                      <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center text-stone-400">{m.temporalConsistencyScore}</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.shortTermConsistency}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.longTermConsistency}%</td>
                        <td className="px-4 py-3 text-center text-blue-400">{m.stabilityIndex}%</td>
                        <td className={`px-4 py-3 text-center font-medium ${m.performanceDrift >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                          {m.performanceDrift >= 0 ? '+' : ''}{m.performanceDrift}%
                        </td>
