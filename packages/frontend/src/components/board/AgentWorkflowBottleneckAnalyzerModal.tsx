import type { AgentWorkflowBottleneckAnalyzerReport } from '../../api/mutations.js';

interface Props {
  result: AgentWorkflowBottleneckAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentWorkflowBottleneckAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing workflow bottlenecks...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No workflow bottleneck data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-emerald-900/20 border border-emerald-500/30 rounded-lg px-4 py-3">
                  <p className="text-emerald-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Bottleneck Score</p>
                  <p className="text-emerald-200 text-xl font-bold">{result.fleetAvgBottleneckScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Critical Bottlenecks</p>
                  <p className="text-red-200 text-xl font-bold">{result.criticalBottlenecks}</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Avg Queue Wait</th>
                      <th className="px-4 py-3 text-center">Stall Freq</th>
                      <th className="px-4 py-3 text-center">Throughput Impact</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-emerald-500" style={{ width: `${m.bottleneckScore}%` }} />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{m.bottleneckScore}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.avgQueueWaitTime}s</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.stallFrequency}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.throughputImpactScore}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.bottleneckEvents}</td>
                        <td className="px-4 py-3 text-center">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${trendBadge(m.trend)}`}>{m.trend}</span>
                        </td>
                        <td className="px-4 py-3 text-center">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${severityBadge(m.severity)}`}>{m.severity}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
