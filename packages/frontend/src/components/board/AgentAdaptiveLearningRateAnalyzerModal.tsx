...
import { AgentAdaptiveLearningRateAnalyzerReport } from '../../api/mutations';

interface Props {
  result: AgentAdaptiveLearningRateAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export function AgentAdaptiveLearningRateAnalyzerModal({ result, loading, onClose }: Props) {
  return (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing adaptive learning rate...</span>
              </div>
            </div>
          ) : !result || result.metrics.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <p className="text-gray-400 text-sm">No adaptive learning rate data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg px-4 py-3">
                  <p className="text-amber-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Learning Score</p>
                  <p className="text-amber-200 text-2xl font-bold">{result.fleetAvgAdaptiveLearningScore}</p>
                </div>
                <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg px-4 py-3">
                  <p className="text-orange-400 text-xs font-medium uppercase tracking-wide mb-1">Slow Learner Agents</p>
                  <p className="text-orange-200 text-2xl font-bold">{result.slowLearnerAgents}</p>
                  <p className="text-gray-500 text-xs mt-1">learning score &lt; 55</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Feedback%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Error Spd</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Generalize%</th>
                  ...
                  <tbody>
                    {result.metrics.map((m) => (
                      <tr key={m.agentId} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-amber-400">{m.adaptiveLearningScore}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.feedbackIncorporationRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.errorCorrectionSpeed}s</td>
                        <td className="py-3 pr-4 text-gray-300">{m.patternGeneralizationRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.behaviorConsistencyRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.totalSessions}</td>
                        <td className="py-3 pr-4">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${trendBadge(m.learningTrend)}`}>
                            {m.learningTrend}
                          </span>
                        </td>
                        <td className="py-3">
