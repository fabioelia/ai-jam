...
import { AgentPriorityAlignmentRateAnalyzerReport } from '../../api/mutations';

interface Props {
  result: AgentPriorityAlignmentRateAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export function AgentPriorityAlignmentRateAnalyzerModal({ result, loading, onClose }: Props) {
  return (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing priority alignment rate...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No priority alignment data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                <div className="bg-green-900/20 border border-green-500/30 rounded-lg px-4 py-3">
                  <p className="text-green-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Score</p>
                  <p className="text-green-200 text-2xl font-bold">{result.fleetAvgAlignmentScore}</p>
                </div>
                ...
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Poor Alignment</p>
                  <p className="text-red-200 text-2xl font-bold">{result.poorAlignmentAgents}</p>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-right text-gray-400 font-medium pb-3 pr-4">Alignment %</th>
                      <th className="text-right text-gray-400 font-medium pb-3 pr-4">High Priority First</th>
                      <th className="text-right text-gray-400 font-medium pb-3 pr-4">Misaligned</th>
                      <th className="text-right text-gray-400 font-medium pb-3 pr-4">Avg Response (ms)</th>
                    ...
                    {result.metrics.map((m) => (
                      <tr key={m.agentId} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-right">
                          <span className="text-green-400 font-semibold">{m.alignmentScore}</span>
                        </td>
                        <td className="py-3 pr-4 text-right text-gray-300">{m.alignmentRate}%</td>
                        <td className="py-3 pr-4 text-right text-gray-300">{m.highPriorityFirst}</td>
                        <td className="py-3 pr-4 text-right text-gray-300">{m.misalignedSessions}</td>
                        <td className="py-3 pr-4 text-right text-gray-300">{m.avgResponseToHighPriority.toLocaleString()}</td>
...
export default AgentPriorityAlignmentRateAnalyzerModal;
