import type { AgentCollaborationEfficiencyAnalyzerReport, AgentCollaborationEfficiencyAnalyzerMetric } from '../../api/mutations.js';

interface Props {
  result: AgentCollaborationEfficiencyAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}

function ratingBadge(rating: AgentCollaborationEfficiencyAnalyzerMetric['rating']): string {
  switch (rating) {
...
function trendBadge(trend: AgentCollaborationEfficiencyAnalyzerMetric['trend']): string {
  switch (trend) {
...
export default function AgentCollaborationEfficiencyAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
      ...
      >
        <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
          <h2 className="text-white font-semibold text-lg flex items-center gap-3">
            ...
            Agent Collaboration Efficiency Analyzer
            {result && (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing collaboration efficiency...</span>
              </div>
            </div>
          ) : !result || result.metrics.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <p className="text-gray-400 text-sm">No collaboration efficiency data found for this project.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-violet-900/20 border border-violet-500/30 rounded-lg px-4 py-3">
                  <p className="text-violet-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Score</p>
                  <p className="text-violet-200 text-xl font-bold">{result.fleetAvgCollaborationScore}%</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Poor Collaborators</p>
                  <p className="text-red-200 text-xl font-bold">{result.poorCollaborators}</p>
                </div>
              ...
              <div className="bg-gray-800/50 border border-gray-700/50 rounded-lg overflow-hidden">
                <div className="px-4 py-2 border-b border-gray-700/50">
                  <h3 className="text-gray-300 text-sm font-medium">Per-Agent Collaboration Efficiency</h3>
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-700/50">
                        ...
                        <th className="text-right px-4 py-2 text-gray-400 font-medium">Handoff</th>
                        <th className="text-right px-4 py-2 text-gray-400 font-medium">Overhead</th>
                        <th className="text-right px-4 py-2 text-gray-400 font-medium">Context Reuse</th>
                          ...
                          <td className="px-4 py-2 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <div className="w-16 bg-gray-700 rounded-full h-1.5">
                                <div className="bg-violet-500 h-1.5 rounded-full" style={{ width: `${m.collaborationScore}%` }} />
                              </div>
                              <span className="text-violet-300 font-medium w-10 text-right">{m.collaborationScore}%</span>
                            </div>
                          </td>
                          <td className="px-4 py-2 text-right text-gray-300">{m.handoffSuccessRate}%</td>
                          <td className="px-4 py-2 text-right text-gray-300">{m.coordinationOverhead}%</td>
                          <td className="px-4 py-2 text-right text-gray-300">{m.sharedContextReuseRate}%</td>
                          <td className="px-4 py-2 text-right text-gray-300">{m.collaborationEvents}</td>
