import type { AgentSessionDurationMetrics, SessionDurationReport } from '../../api/mutations.js';

interface AgentSessionDurationAnalyzerModalProps {
  result: SessionDurationReport | null;
  loading: boolean;
  onClose: () => void;
}

function tierBadgeClass(tier: AgentSessionDurationMetrics['durationTier']): string {
  switch (tier) {
...
function tierLabel(tier: AgentSessionDurationMetrics['durationTier']): string {
  switch (tier) {
...
export default function AgentSessionDurationAnalyzerModal({
  result,
  loading,
  onClose,
}: AgentSessionDurationAnalyzerModalProps) {
  return (
    <div
            ...
            <>
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Slow</p>
                ...
                <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg px-4 py-3">
                  ...
                  <p className="text-amber-200 text-xl font-bold">{formatMs(result.summary.avgDurationMs)}</p>
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {result.agents.map((agent, i) => (
                      <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center text-gray-300 text-xs">{formatMs(agent.avgDurationMs)}</td>
                        <td className="px-4 py-3 text-center text-gray-300 text-xs">{formatMs(agent.medianDurationMs)}</td>
                        <td className="px-4 py-3 text-center text-gray-300 text-xs">{formatMs(agent.p95DurationMs)}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div
                                className="h-1.5 rounded-full bg-amber-500"
                                style={{ width: `${Math.min(100, agent.durationScore)}%` }}
                              />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{agent.durationScore.toFixed(1)}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${tierBadgeClass(agent.durationTier)}`}>
                            {tierLabel(agent.durationTier)}
                          </span>
                        </td>
                      </tr>
                    ))}
