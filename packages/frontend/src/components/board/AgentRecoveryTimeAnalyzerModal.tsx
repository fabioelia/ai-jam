...
import { AgentRecoveryTimeAnalyzerReport } from '../../api/mutations';

interface Props {
  result: AgentRecoveryTimeAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export function AgentRecoveryTimeAnalyzerModal({ result, loading, onClose }: Props) {
  return (
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <p className="text-gray-400 text-sm">No recovery time data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-indigo-900/20 border border-indigo-500/30 rounded-lg px-4 py-3">
                  <p className="text-indigo-400 text-xs font-medium uppercase tracking-wide mb-1">Avg Recovery Time</p>
                  <p className="text-indigo-200 text-2xl font-bold">{result.avgRecoveryTimeSeconds}s</p>
                </div>
                <div className="bg-green-900/20 border border-green-500/30 rounded-lg px-4 py-3">
                  <p className="text-green-400 text-xs font-medium uppercase tracking-wide mb-1">Recovery Rate</p>
                  <p className="text-green-200 text-2xl font-bold">{result.recoveryRate}%</p>
                </div>
                <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg px-4 py-3">
                  <p className="text-orange-400 text-xs font-medium uppercase tracking-wide mb-1">Total Failures</p>
                  <p className="text-orange-200 text-2xl font-bold">{result.totalFailures}</p>
                </div>
                <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg px-4 py-3">
                  <p className="text-blue-400 text-xs font-medium uppercase tracking-wide mb-1">Recovered</p>
                  <p className="text-blue-200 text-2xl font-bold">{result.recoveredCount}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Failed Recoveries</p>
                  <p className="text-red-200 text-2xl font-bold">{result.failedRecoveryCount}</p>
                </div>
                <div className="bg-gray-800/40 border border-gray-700 rounded-lg px-4 py-3">
                  <p className="text-gray-400 text-xs font-medium uppercase tracking-wide mb-1">Max Recovery Time</p>
                  <p className="text-white text-2xl font-bold">{result.maxRecoveryTimeSeconds}s</p>
                </div>
              </div>
              <div className="flex gap-4 text-sm">
                <div className="bg-gray-800/40 border border-gray-700 rounded-lg px-3 py-2 flex gap-2 items-center">
                  ...
                  <span className="text-indigo-400 font-medium">{result.fastestRecoveryAgent}</span>
                </div>
                <div className="bg-gray-800/40 border border-gray-700 rounded-lg px-3 py-2 flex gap-2 items-center">
                  <span className="text-gray-400">Slowest:</span>
                  <span className="text-red-400 font-medium">{result.slowestRecoveryAgent}</span>
                </div>
                <div className="bg-gray-800/40 border border-gray-700 rounded-lg px-3 py-2 flex gap-2 items-center">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Avg Recovery</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Max Recovery</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Recovery%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Failures</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Recovered</th>
                  ...
                  <tbody>
                    {result.metrics.map((m) => (
                      <tr key={m.agentId} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-indigo-400">{m.avgRecoveryTimeSeconds}s</td>
                        <td className="py-3 pr-4 text-gray-300">{m.maxRecoveryTimeSeconds}s</td>
                        <td className="py-3 pr-4 text-gray-300">{m.recoveryRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.totalFailureEvents}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.recoveredCount}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.totalSessions}</td>
                        <td className="py-3 pr-4">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${trendBadge(m.recoveryTrend)}`}>
                            {m.recoveryTrend}
                          </span>
                        </td>
                        <td className="py-3">
