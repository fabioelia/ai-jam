import type { AgentScopeCreepDetectorReport } from '../../api/mutations.js';

interface Props {
  result: AgentScopeCreepDetectorReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentScopeCreepDetectorModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Detecting scope creep...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No scope creep data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-teal-900/20 border border-teal-500/30 rounded-lg px-4 py-3">
                  <p className="text-teal-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Scope Creep Score</p>
                  <p className="text-teal-200 text-xl font-bold">{result.fleetAvgScopeCreepScore}</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Scope Creep Score</th>
                      <th className="px-4 py-3 text-center">Overrun Ratio</th>
                      <th className="px-4 py-3 text-center">Out of Scope</th>
                      <th className="px-4 py-3 text-center">Resource Overuse</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-teal-500" style={{ width: `${Math.min(100, m.scopeCreepScore)}%` }} />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{m.scopeCreepScore}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.avgOverrunRatio}x</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.outOfScopeTaskRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.resourceOveruseRate}%</td>
