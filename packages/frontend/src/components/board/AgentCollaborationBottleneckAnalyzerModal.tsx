...
import { AgentCollaborationBottleneckAnalyzerReport } from '../../api/mutations';

interface Props {
  result: AgentCollaborationBottleneckAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export function AgentCollaborationBottleneckAnalyzerModal({ result, loading, onClose }: Props) {
  return (
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl shadow-2xl w-full max-w-5xl max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <h2 className="text-lg font-semibold text-white flex items-center gap-2">
            ...
            Collaboration Bottleneck Analyzer
            {result && (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing collaboration bottlenecks...</span>
              </div>
            </div>
          ) : !result || result.metrics.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <p className="text-gray-400 text-sm">No collaboration bottleneck data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-green-900/20 border border-green-500/30 rounded-lg px-4 py-3">
                  <p className="text-green-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Collaboration Score</p>
                  <p className="text-green-200 text-2xl font-bold">{result.fleetAvgCollaborationScore}</p>
                </div>
                <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg px-4 py-3">
                  <p className="text-orange-400 text-xs font-medium uppercase tracking-wide mb-1">Bottleneck Agents</p>
                  <p className="text-orange-200 text-2xl font-bold">{result.bottleneckAgents}</p>
                  <p className="text-gray-500 text-xs mt-1">bottleneck freq &gt; 30%</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Handoff%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Context%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Bottleneck%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Clarify%</th>
                  ...
                  <tbody>
                    {result.metrics.map((m) => (
                      <tr key={m.agentId} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-green-400">{m.collaborationScore}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.handoffSuccessRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.contextAbsorptionRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.bottleneckFrequency}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.clarificationRequestRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.totalSessions}</td>
                        <td className="py-3 pr-4">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${trendBadge(m.collaborationTrend)}`}>
                            {m.collaborationTrend}
                          </span>
                        </td>
                        <td className="py-3">
