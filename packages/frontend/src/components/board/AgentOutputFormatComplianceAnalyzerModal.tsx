import type { AgentOutputFormatComplianceAnalyzerReport } from '../../api/mutations.js';

interface Props {
  result: AgentOutputFormatComplianceAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentOutputFormatComplianceAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing output format compliance...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No output format compliance data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-900/20 border border-gray-500/30 rounded-lg px-4 py-3">
                  <p className="text-gray-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Compliance Score</p>
                  <p className="text-gray-200 text-2xl font-bold">{result.fleetAvgComplianceScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">High Violation Agents</p>
                  <p className="text-red-200 text-xl font-bold flex items-center gap-2">
                    {result.highViolationAgents}
                    {result.highViolationAgents > 0 && (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Top Violation</th>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {result.metrics.map((m, i) => (
                      <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center text-gray-400">{m.avgComplianceScore}</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.complianceRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.partialComplianceRate}%</td>
                        <td className="px-4 py-3 text-center text-orange-400">{m.formatViolations}</td>
                        <td className="px-4 py-3 text-center text-gray-400 text-xs font-mono">{m.mostCommonViolation}</td>
