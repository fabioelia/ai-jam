...
import { AgentCrossDomainTransferAnalyzerReport } from '../../api/mutations';

interface Props {
  result: AgentCrossDomainTransferAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export function AgentCrossDomainTransferAnalyzerModal({ result, loading, onClose }: Props) {
  return (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing cross-domain transfer...</span>
              </div>
            </div>
          ) : !result || result.metrics.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <p className="text-gray-400 text-sm">No cross-domain transfer data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-violet-900/20 border border-violet-500/30 rounded-lg px-4 py-3">
                  <p className="text-violet-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Transfer Score</p>
                  <p className="text-violet-200 text-2xl font-bold">{result.fleetAvgTransferScore}</p>
                </div>
                <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg px-4 py-3">
                  <p className="text-orange-400 text-xs font-medium uppercase tracking-wide mb-1">Narrow Specialist Agents</p>
                  <p className="text-orange-200 text-2xl font-bold">{result.narrowSpecialistAgents}</p>
                  <p className="text-gray-500 text-xs mt-1">specialization index &gt; 70</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Diversity</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Consistency</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Adaptation</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Specialization</th>
                  ...
                  <tbody>
                    {result.metrics.map((m) => (
                      <tr key={m.agentId} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-violet-400">{m.transferScore}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.domainDiversityScore}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.crossDomainConsistency}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.adaptationRate}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.specializationIndex}%</td>
