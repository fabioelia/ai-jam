import type { AgentInstructionParseAccuracyReport } from '../../api/mutations.js';

interface Props {
  result: AgentInstructionParseAccuracyReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentInstructionParseAccuracyAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing instruction parse accuracy...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No instruction parse data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg px-4 py-3">
                  <p className="text-amber-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Accuracy</p>
                  <p className="text-amber-200 text-xl font-bold">{result.fleetAvgAccuracyScore}%</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Total Instrs</th>
                      <th className="px-4 py-3 text-center">Correct</th>
                      <th className="px-4 py-3 text-center">Misinterpreted</th>
                      <th className="px-4 py-3 text-center">Clarifications</th>
                      <th className="px-4 py-3 text-center">Avg Rounds</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-amber-500" style={{ width: `${m.accuracyScore}%` }} />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{m.accuracyScore}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.totalInstructions}</td>
                        <td className="px-4 py-3 text-center text-green-400">{m.correctlyParsed}</td>
                        <td className="px-4 py-3 text-center text-red-400">{m.misinterpreted}</td>
                        <td className="px-4 py-3 text-center text-yellow-400">{m.clarificationRequests}</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.avgClarificationRounds}</td>
