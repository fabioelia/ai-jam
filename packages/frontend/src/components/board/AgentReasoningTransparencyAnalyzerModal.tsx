...
import { AgentReasoningTransparencyAnalyzerReport } from '../../api/mutations';

interface Props {
  result: AgentReasoningTransparencyAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export function AgentReasoningTransparencyAnalyzerModal({ result, loading, onClose }: Props) {
  return (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing reasoning transparency...</span>
              </div>
            </div>
          ) : !result || result.metrics.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <p className="text-gray-400 text-sm">No transparency data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-gray-800/40 border border-gray-500/30 rounded-lg px-4 py-3">
                  <p className="text-gray-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Transparency</p>
                  <p className="text-gray-200 text-2xl font-bold">{result.fleetAvgTransparencyScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Low Transparency Agents</p>
                  <p className="text-red-200 text-2xl font-bold">{result.lowTransparencyAgents}</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Exposure%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Explainability</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Audit%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Depth</th>
                  ...
                  <tbody>
                    {result.metrics.map((m) => (
                      <tr key={m.agentId} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-gray-300">{m.transparencyScore}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.reasoningExposureRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.explainabilityIndex}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.auditabilityRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.reasoningDepthScore}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.totalSessions}</td>
                        <td className="py-3 pr-4">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${trendBadge(m.transparencyTrend)}`}>
                            {m.transparencyTrend}
                          </span>
                        </td>
                        <td className="py-3">
