...
import { AgentInitiativeCalibrationAnalyzerReport } from '../../api/mutations';

interface Props {
  result: AgentInitiativeCalibrationAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export function AgentInitiativeCalibrationAnalyzerModal({ result, loading, onClose }: Props) {
  return (
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl shadow-2xl w-full max-w-5xl max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <h2 className="text-lg font-semibold text-white flex items-center gap-2">
            ...
            {result && (
              <span className="text-sm font-normal text-rose-400/80 border border-rose-500/20 bg-rose-500/10 px-2 py-0.5 rounded-full">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing initiative calibration...</span>
              </div>
            </div>
            ...
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-rose-500/10 border border-rose-500/20 rounded-lg px-4 py-3">
                  <p className="text-rose-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Calibration</p>
                  <p className="text-rose-200 text-2xl font-bold">{result.fleetAvgCalibrationScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Poorly Calibrated</p>
                  <p className="text-red-200 text-2xl font-bold">{result.poorlyCalibrated}</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Avg Duration</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Variance</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">OverReach%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">UnderReach%</th>
                  ...
                  <tbody>
                    {result.metrics.map((m) => (
                      <tr key={m.agentId} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-rose-300 font-semibold">{m.calibrationScore}</td>
                        <td className="py-3 pr-4 text-gray-300">{(m.avgSessionDuration / 1000).toFixed(1)}s</td>
                        <td className="py-3 pr-4 text-gray-300">{(m.durationVariance / 1000).toFixed(1)}s</td>
                        <td className="py-3 pr-4 text-gray-300">{m.overReachRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.underReachRate}%</td>
