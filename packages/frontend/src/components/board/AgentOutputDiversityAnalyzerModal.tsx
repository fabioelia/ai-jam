...
import { AgentOutputDiversityAnalyzerReport } from '../../api/mutations';

interface Props {
  result: AgentOutputDiversityAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export function AgentOutputDiversityAnalyzerModal({ result, loading, onClose }: Props) {
  return (
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl shadow-2xl w-full max-w-5xl max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <h2 className="text-lg font-semibold text-white flex items-center gap-2">
            ...
            Output Diversity
            {result && (
              <span className="text-sm font-normal text-amber-400/80 border border-amber-500/20 bg-amber-500/10 px-2 py-0.5 rounded-full">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing output diversity...</span>
              </div>
            </div>
          ) : !result || result.metrics.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <p className="text-gray-400 text-sm">No diversity data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg px-4 py-3">
                  <p className="text-amber-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Diversity</p>
                  <p className="text-amber-200 text-2xl font-bold">{result.fleetAvgDiversityScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Low Diversity Agents</p>
                  <p className="text-red-200 text-2xl font-bold">{result.lowDiversityAgents}</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Unique Statuses</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Repetition%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Adaptability</th>
                  ...
                  <tbody>
                    {result.metrics.map((m) => (
                      <tr key={m.agentId} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-amber-300 font-semibold">{m.diversityScore}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.uniqueStatusCount}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.repetitionRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.adaptabilityScore}</td>
