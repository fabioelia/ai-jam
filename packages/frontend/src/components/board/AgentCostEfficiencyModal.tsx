import type { AgentCostEfficiencyReport, AgentCostEfficiencyMetrics } from '../../api/mutations.js';

interface Props {
  result: AgentCostEfficiencyReport | null;
  isOpen: boolean;
  loading: boolean;
  onClose: () => void;
}

function tierBadgeClass(tier: AgentCostEfficiencyMetrics['efficiencyTier']): string {
  switch (tier) {
...
export default function AgentCostEfficiencyModal({ result, isOpen, loading, onClose }: Props) {
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing agent cost efficiency...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No agent session data found for this project.</p>
            </div>
          ) : (
            <>
              {/* Summary cards */}
              <div className="grid grid-cols-4 gap-3">
                <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg px-4 py-3">
                  <p className="text-amber-400 text-xs font-medium uppercase tracking-wide mb-1">Total Tokens Used</p>
                  <p className="text-amber-200 text-sm font-semibold">{formatTokens(result.totalTokensUsed)}</p>
                </div>
                <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg px-4 py-3">
                  <p className="text-orange-400 text-xs font-medium uppercase tracking-wide mb-1">Avg Efficiency Score</p>
                  <p className="text-orange-200 text-sm font-semibold">{result.avgCostEfficiencyScore}/100</p>
                </div>
                ...
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Least Efficient Agent</p>
                  <p className="text-red-200 text-sm font-semibold truncate">{result.leastEfficientAgent ?? '—'}</p>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Avg Tokens/Session</th>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {result.agentMetrics.map((agent, i) => (
                      <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center text-gray-300">{agent.completedSessions}</td>
                        <td className="px-4 py-3 text-center text-gray-400 font-mono text-xs">{formatTokens(agent.avgTokensPerSession)}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div
                                className="h-1.5 rounded-full bg-amber-500"
                                style={{ width: `${agent.costEfficiencyScore}%` }}
                              />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{agent.costEfficiencyScore}</span>
                          </div>
                        </td>
