import type { AgentGoalCompletionRateAnalyzerReport } from '../../api/mutations.js';

interface Props {
  result: AgentGoalCompletionRateAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentGoalCompletionRateAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing goal completion rates...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No goal completion data found for this project.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-violet-900/20 border border-violet-500/30 rounded-lg px-4 py-3">
                  <p className="text-violet-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Completion</p>
                  <p className="text-violet-200 text-2xl font-bold">{result.fleetAvgGoalCompletionRate}%</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Low Completion Agents</p>
                  <p className="text-red-200 text-2xl font-bold">{result.lowCompletionAgents}</p>
                </div>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-violet-500" style={{ width: `${m.goalCompletionRate}%` }} />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{m.goalCompletionRate}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.totalGoals}</td>
                        <td className="px-4 py-3 text-center text-green-400">{m.completedGoals}</td>
                        <td className="px-4 py-3 text-center text-red-400">{m.abandonedGoals}</td>
                        <td className="px-4 py-3 text-center text-yellow-400">{m.partialGoals}</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.avgCompletionTime}</td>
