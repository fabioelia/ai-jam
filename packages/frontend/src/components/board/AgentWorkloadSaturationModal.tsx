import type { WorkloadSaturationReport } from '../../api/mutations.js';

interface AgentWorkloadSaturationModalProps {
  result: WorkloadSaturationReport | null;
  loading: boolean;
  onClose: () => void;
}
...
function tierBadgeClass(tier: string): string {
  switch (tier) {
    case 'overloaded': return 'bg-red-900/30 text-red-400 border-red-700/30';
    case 'saturated': return 'bg-orange-900/30 text-orange-400 border-orange-700/30';
    case 'healthy': return 'bg-green-900/30 text-green-400 border-green-700/30';
    case 'underutilized': return 'bg-blue-900/30 text-blue-400 border-blue-700/30';
...
export default function AgentWorkloadSaturationModal({ result, loading, onClose }: AgentWorkloadSaturationModalProps) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing workload saturation...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No workload saturation data found for this project.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg px-4 py-3">
                  <p className="text-amber-400 text-xs font-medium uppercase tracking-wide mb-1">Avg Saturation</p>
                  <p className="text-amber-200 text-xl font-bold">{formatRate(result.summary.avgSaturationRate)}</p>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Saturation Score</th>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {result.agents.map((metric, i) => (
                      <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center text-gray-300">{metric.activeTickets}</td>
                        <td className="px-4 py-3 text-center text-gray-300">{metric.inProgressTickets}</td>
                        <td className="px-4 py-3 text-center text-gray-300">{metric.reviewTickets}</td>
                        <td className="px-4 py-3 text-center text-gray-300">{formatRate(metric.saturationRate)}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div
                                className="h-1.5 rounded-full bg-amber-500"
                                style={{ width: `${metric.saturationScore}%` }}
                              />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{metric.saturationScore}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${tierBadgeClass(metric.saturationTier)}`}>
                            {tierLabel(metric.saturationTier)}
                          </span>
                        </td>
                      </tr>
                    ))}
