import type { AgentKnowledgeBoundaryMappingAnalyzerReport } from '../../api/mutations.js';

interface Props {
  result: AgentKnowledgeBoundaryMappingAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentKnowledgeBoundaryMappingAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing knowledge boundary mapping...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No knowledge boundary data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg px-4 py-3">
                  <p className="text-blue-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Boundary Score</p>
                  <p className="text-blue-200 text-2xl font-bold">{result.fleetAvgBoundaryScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Overconfident Agents</p>
                  <p className="text-red-200 text-xl font-bold flex items-center gap-2">
                    {result.overconfidentAgents}
                    {result.overconfidentAgents > 0 && (
                      <span className="text-xs bg-red-500/20 border border-red-500/30 text-red-400 px-1.5 py-0.5 rounded-full">high failure</span>
                    )}
                  </p>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Silent Fail%</th>
                      <th className="px-4 py-3 text-center">Boundary Resp%</th>
                      <th className="px-4 py-3 text-center">Gap Density</th>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {result.metrics.map((m, i) => (
                      <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center text-blue-400">{m.boundaryScore}</td>
                        <td className="px-4 py-3 text-center text-green-400">{m.knowledgeCoverageScore}%</td>
                        <td className="px-4 py-3 text-center text-red-400">{m.silentFailureRate}%</td>
                        <td className="px-4 py-3 text-center text-cyan-400">{m.boundaryRespectRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-400">{m.gapDensity}</td>
