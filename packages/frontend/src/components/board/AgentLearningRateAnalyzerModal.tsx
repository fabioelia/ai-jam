import type { AgentLearningRateAnalyzerReport } from '../../api/mutations.js';

interface Props {
  result: AgentLearningRateAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
function levelBadge(level: string): string {
  switch (level) {
    case 'rapid': return 'bg-green-900/30 text-green-400 border-green-700/30';
    case 'steady': return 'bg-blue-900/30 text-blue-400 border-blue-700/30';
...
export default function AgentLearningRateAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing agent learning rates...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No learning rate data found for this project.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-lime-900/20 border border-lime-500/30 rounded-lg px-4 py-3">
                  <p className="text-lime-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Learning Score</p>
                  <p className="text-lime-200 text-2xl font-bold">{result.fleetAvgLearningScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Stagnant Agents</p>
                  <p className="text-red-200 text-2xl font-bold">{result.stagnantAgents}</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Repeat Error %</th>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {result.metrics.map((m, i) => (
                      <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-lime-500" style={{ width: `${m.learningScore}%` }} />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{m.learningScore}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.improvementRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.repeatErrorRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.sessionSuccessProgression}</td>
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${levelBadge(m.learningLevel)}`}>{m.learningLevel}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
