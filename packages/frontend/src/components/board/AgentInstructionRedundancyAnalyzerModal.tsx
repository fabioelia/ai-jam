...
import { AgentInstructionRedundancyAnalyzerReport } from '../../api/mutations';

interface Props {
  result: AgentInstructionRedundancyAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export function AgentInstructionRedundancyAnalyzerModal({ result, loading, onClose }: Props) {
  return (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing instruction redundancy...</span>
              </div>
            </div>
          ) : !result || result.metrics.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <p className="text-gray-400 text-sm">No instruction redundancy data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-rose-900/20 border border-rose-500/30 rounded-lg px-4 py-3">
                  <p className="text-rose-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Redundancy Score</p>
                  <p className="text-rose-200 text-2xl font-bold">{result.fleetAvgRedundancyScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">High Redundancy Agents</p>
                  <p className="text-red-200 text-2xl font-bold">{result.highRedundancyAgents}</p>
                  <p className="text-gray-500 text-xs mt-1">repeat rate &gt; 35%</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Unique %</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Repeat %</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Depth</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Context Ret.</th>
                  ...
                  <tbody>
                    {result.metrics.map((m) => (
                      <tr key={m.agentId} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-rose-400">{m.redundancyScore}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.uniqueInstructionRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.repeatInstructionRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.avgRedundancyDepth}</td>
                        ...
                        <td className="py-3 pr-4">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${trendBadge(m.redundancyTrend)}`}>
                            {m.redundancyTrend}
                          </span>
                        </td>
                        <td className="py-3">
