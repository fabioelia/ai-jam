import type { AgentGoalAlignmentReport } from '../../api/mutations.js';

interface Props {
  result: AgentGoalAlignmentReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentGoalAlignmentScoreModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing goal alignment...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No goal alignment data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-purple-900/20 border border-purple-500/30 rounded-lg px-4 py-3">
                  <p className="text-purple-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Alignment</p>
                  <p className="text-purple-200 text-xl font-bold">{result.fleetAvgAlignmentScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Low Alignment Agents</p>
                  <p className="text-red-200 text-xl font-bold">{result.lowAlignmentAgents}</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Goal Completion</th>
                      <th className="px-4 py-3 text-center">Deviation Rate</th>
                      <th className="px-4 py-3 text-center">Abandonment</th>
                      <th className="px-4 py-3 text-center">Aligned Sessions</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-purple-500" style={{ width: `${m.alignmentScore}%` }} />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{m.alignmentScore}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.goalCompletionRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.deviationRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.midTaskAbandonmentRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.alignedSessions}</td>
