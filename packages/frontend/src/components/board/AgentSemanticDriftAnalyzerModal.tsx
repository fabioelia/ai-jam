import type { AgentSemanticDriftReport } from '../../api/mutations.js';

interface Props {
  result: AgentSemanticDriftReport | null;
  loading: boolean;
  onClose: () => void;
}
...
function ratingBadge(rating: string): string {
  switch (rating) {
    ...
    case 'mild': return 'bg-blue-900/30 text-blue-400 border-blue-700/30';
    case 'moderate': return 'bg-yellow-900/30 text-yellow-400 border-yellow-700/30';
    case 'severe': return 'bg-red-900/30 text-red-400 border-red-700/30';
...
export default function AgentSemanticDriftAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing semantic drift...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No semantic drift data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-4 gap-3">
                <div className="bg-indigo-900/20 border border-indigo-500/30 rounded-lg px-4 py-3">
                  <p className="text-indigo-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Drift Score</p>
                  <p className="text-indigo-200 text-xl font-bold">{result.fleetAvgDriftScore}</p>
                </div>
                <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg px-4 py-3">
                  <p className="text-orange-400 text-xs font-medium uppercase tracking-wide mb-1">Drift Rate</p>
                  <p className="text-orange-200 text-xl font-bold">{result.driftRate}%</p>
                </div>
                <div className="bg-yellow-900/20 border border-yellow-500/30 rounded-lg px-4 py-3">
                  <p className="text-yellow-400 text-xs font-medium uppercase tracking-wide mb-1">Avg Magnitude</p>
                  <p className="text-yellow-200 text-xl font-bold">{result.averageDriftMagnitude}</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-800/40 border border-gray-700 rounded-lg px-4 py-3">
                  <p className="text-gray-400 text-xs font-medium uppercase tracking-wide mb-1">Most Stable</p>
                  <p className="text-green-300 text-sm font-semibold">{result.mostStableAgent}</p>
                </div>
                <div className="bg-gray-800/40 border border-gray-700 rounded-lg px-4 py-3">
                  <p className="text-gray-400 text-xs font-medium uppercase tracking-wide mb-1">Most Drifted</p>
                  <p className="text-red-300 text-sm font-semibold">{result.mostDriftedAgent}</p>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Drift Rate</th>
                      <th className="px-4 py-3 text-center">Avg Magnitude</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-indigo-500" style={{ width: `${m.driftScore}%` }} />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{m.driftScore}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.totalSessions}</td>
                        <td className="px-4 py-3 text-center text-orange-400">{m.sessionsWithDrift}</td>
                        <td className="px-4 py-3 text-center text-yellow-400">{m.driftRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.averageDriftMagnitude}</td>
