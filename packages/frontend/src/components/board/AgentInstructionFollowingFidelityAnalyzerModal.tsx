...
import { AgentInstructionFollowingFidelityAnalyzerReport } from '../../api/mutations';

interface Props {
  result: AgentInstructionFollowingFidelityAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export function AgentInstructionFollowingFidelityAnalyzerModal({ result, loading, onClose }: Props) {
  return (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing instruction following fidelity...</span>
              </div>
            </div>
          ) : !result || result.metrics.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <p className="text-gray-400 text-sm">No instruction following fidelity data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-slate-900/20 border border-slate-500/30 rounded-lg px-4 py-3">
                  <p className="text-slate-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Fidelity Score</p>
                  <p className="text-slate-200 text-2xl font-bold">{result.fleetAvgFidelityScore}</p>
                </div>
                <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg px-4 py-3">
                  <p className="text-orange-400 text-xs font-medium uppercase tracking-wide mb-1">Low Fidelity Agents</p>
                  <p className="text-orange-200 text-2xl font-bold">{result.lowFidelityAgents}</p>
                  <p className="text-gray-500 text-xs mt-1">fidelity score &lt; 60</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Fidelity</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Compliance%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Omission%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Addition%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Interpretation%</th>
                  ...
                  <tbody>
                    {result.metrics.map((m) => (
                      <tr key={m.agentId} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-slate-400">{m.fidelityScore}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.exactComplianceRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.omissionRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.additionRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.interpretationAccuracy}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.totalSessions}</td>
                        <td className="py-3 pr-4">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${trendBadge(m.fidelityTrend)}`}>
                            {m.fidelityTrend}
                          </span>
                        </td>
                        <td className="py-3">
