import type { AgentTokenCostReport } from '../../api/mutations.js';

interface AgentTokenCostEfficiencyModalProps {
  projectId: string;
  result: AgentTokenCostReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentTokenCostEfficiencyModal({
  result,
  loading,
  onClose,
}: AgentTokenCostEfficiencyModalProps) {
  const avgTokensPerTicket =
    result && result.agents.length > 0
      ? Math.round(result.agents.reduce((s, a) => s + a.tokensPerTicket, 0) / result.agents.length)
      : 0;
          ...
          <h2 className="text-white font-semibold text-lg flex items-center gap-3">
            ...
            {result && (
              <span className="text-sm font-normal text-emerald-400/80 border border-emerald-500/30 bg-emerald-500/10 px-2 py-0.5 rounded-full">
                avg score {result.summary.avgEfficiencyScore}
              </span>
            )}
          </h2>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing token cost efficiency...</span>
              </div>
            </div>
          ) : !result || result.agents.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No token cost data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="bg-emerald-900/20 border border-emerald-500/30 rounded-lg px-4 py-3">
                  <p className="text-emerald-400 text-xs font-medium uppercase tracking-wide mb-1">Total Est. Cost $</p>
                  <p className="text-emerald-200 text-xl font-bold">{formatCost(result.summary.totalEstimatedCost)}</p>
                </div>
                <div className="bg-emerald-900/20 border border-emerald-500/30 rounded-lg px-4 py-3">
                  <p className="text-emerald-400 text-xs font-medium uppercase tracking-wide mb-1">Avg Tokens/Ticket</p>
                  <p className="text-emerald-200 text-xl font-bold">{avgTokensPerTicket.toLocaleString()}</p>
                </div>
                <div className="bg-emerald-900/20 border border-emerald-500/30 rounded-lg px-4 py-3">
                  <p className="text-emerald-400 text-xs font-medium uppercase tracking-wide mb-1">Avg Efficiency Score</p>
                  <p className="text-emerald-200 text-xl font-bold">{result.summary.avgEfficiencyScore}/100</p>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Tokens In</th>
                      <th className="px-4 py-3 text-center">Tokens Out</th>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {result.agents.map((agent, i) => (
                      <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center text-gray-300">{agent.totalTokensIn.toLocaleString()}</td>
                        <td className="px-4 py-3 text-center text-gray-300">{agent.totalTokensOut.toLocaleString()}</td>
                        <td className="px-4 py-3 text-center text-gray-300 font-mono text-xs">{formatCost(agent.estimatedCostUsd)}</td>
                        <td className="px-4 py-3 text-center text-gray-300">{agent.ticketsCompleted}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            ...
                            <span className="text-gray-200 font-mono text-xs w-6">{agent.efficiencyScore}</span>
