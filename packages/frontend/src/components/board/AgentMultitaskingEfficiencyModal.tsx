...
function scoreBarColor(tier: AgentMultitaskingProfile['efficiencyTier']): string {
  switch (tier) {
    case 'optimal': return 'from-green-500 to-amber-400';
    case 'acceptable': return 'from-blue-500 to-amber-400';
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
          <h2 className="text-white font-semibold text-lg flex items-center gap-3">
            ...
            AI Agent Multi-Tasking Efficiency Analyzer
            {result && (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing agent multi-tasking efficiency...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No agent multi-tasking data found for this project.</p>
            </div>
          ) : (
            <>
              {/* Summary cards */}
              <div className="grid grid-cols-4 gap-3">
                <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg px-4 py-3">
                  <p className="text-amber-400 text-xs font-medium uppercase tracking-wide mb-1">System Avg Concurrency</p>
                  <p className="text-amber-200 text-sm font-semibold">{result.systemAvgConcurrency.toFixed(2)}</p>
                </div>
                ...
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  ...
                  <p className="text-red-200 text-sm font-semibold">{result.mostOverloadedAgent ?? '—'}</p>
                </div>
                <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg px-4 py-3">
                  <p className="text-orange-400 text-xs font-medium uppercase tracking-wide mb-1">Recommended Max Concurrency</p>
                  <p className="text-orange-200 text-sm font-semibold">{result.recommendedMaxConcurrency}</p>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Avg Concurrency</th>
                      <th className="px-4 py-3 text-center">Peak</th>
                      <th className="px-4 py-3 text-center">Optimal</th>
                      <th className="px-4 py-3 text-center">Overloaded %</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-800">
                    {result.agents.map((agent, i) => (
                      <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div
                                className={`h-1.5 rounded-full bg-gradient-to-r ${scoreBarColor(agent.efficiencyTier)}`}
                        ...
                        <td className="px-4 py-3 text-center text-gray-400">{agent.peakConcurrency}</td>
                        <td className="px-4 py-3 text-center text-gray-400">{agent.optimalConcurrency}</td>
