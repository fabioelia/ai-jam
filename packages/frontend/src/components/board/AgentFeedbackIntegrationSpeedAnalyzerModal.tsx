import type { AgentFeedbackIntegrationSpeedAnalyzerReport } from '../../api/mutations.js';

interface Props {
  result: AgentFeedbackIntegrationSpeedAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentFeedbackIntegrationSpeedAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
      ...
      >
        <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
          <h2 className="text-white font-semibold text-lg flex items-center gap-3">
            ...
            Agent Feedback Integration Speed Analyzer
            {result && (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing feedback integration speed...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No feedback integration speed data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg px-4 py-3">
                  <p className="text-orange-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Integration Score</p>
                  <p className="text-orange-200 text-xl font-bold">{result.fleetAvgIntegrationScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Slow Learners</p>
                  <p className="text-red-200 text-xl font-bold">{result.slowLearners}</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Integration Score</th>
                      <th className="px-4 py-3 text-center">Avg Speed (ms)</th>
                      <th className="px-4 py-3 text-center">Response Rate</th>
                      <th className="px-4 py-3 text-center">Repeat Mistakes</th>
                      <th className="px-4 py-3 text-center">Feedback Events</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-orange-500" style={{ width: `${Math.min(100, m.integrationScore)}%` }} />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{m.integrationScore}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.avgIntegrationSpeedMs.toLocaleString()}</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.feedbackResponseRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.repeatMistakeRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.totalFeedbackEvents}</td>
