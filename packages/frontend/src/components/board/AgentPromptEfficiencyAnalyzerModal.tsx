import type { AgentPromptEfficiencyAnalyzerReport } from '../../api/mutations.js';

interface Props {
  result: AgentPromptEfficiencyAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentPromptEfficiencyAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing prompt efficiency...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No prompt efficiency data found for this project.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg px-4 py-3">
                  <p className="text-amber-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Efficiency</p>
                  <p className="text-amber-200 text-2xl font-bold">{result.fleetAvgPromptEfficiencyScore}%</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Low Efficiency Agents</p>
                  <p className="text-red-200 text-2xl font-bold">{result.lowEfficiencyAgents}</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Avg Tokens/Task</th>
                      <th className="px-4 py-3 text-center">Tasks/K Tokens</th>
                      <th className="px-4 py-3 text-center">Verbosity %</th>
                      <th className="px-4 py-3 text-center">Concisency %</th>
                      <th className="px-4 py-3 text-center">Total Tokens Est.</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-amber-500" style={{ width: `${m.promptEfficiencyScore}%` }} />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{m.promptEfficiencyScore}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.avgTokensPerTask.toLocaleString()}</td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.avgTasksPerKTokens}</td>
                        <td className="px-4 py-3 text-center text-orange-400">{m.verbosityRate}%</td>
                        <td className="px-4 py-3 text-center text-green-400">{m.concisencyRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-400 text-xs">{m.totalTokensEstimate.toLocaleString()}</td>
