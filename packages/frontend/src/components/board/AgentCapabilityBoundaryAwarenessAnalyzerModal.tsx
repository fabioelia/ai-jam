import type { AgentCapabilityBoundaryAwarenessAnalyzerReport } from '../../api/mutations.js';

interface Props {
  result: AgentCapabilityBoundaryAwarenessAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentCapabilityBoundaryAwarenessAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing capability boundary awareness...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No capability boundary awareness data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-purple-900/20 border border-purple-500/30 rounded-lg px-4 py-3">
                  <p className="text-purple-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Boundary Awareness Score</p>
                  <p className="text-purple-200 text-2xl font-bold">{result.fleetAvgBoundaryAwarenessScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Poor Boundary Agents</p>
                  <p className="text-red-200 text-xl font-bold flex items-center gap-2">
                    {result.poorBoundaryAgents}
                    {result.poorBoundaryAgents > 0 && (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Awareness</th>
                      <th className="px-4 py-3 text-center">Overreach%</th>
                      <th className="px-4 py-3 text-center">Underreach%</th>
                      <th className="px-4 py-3 text-center">Routing%</th>
                      <th className="px-4 py-3 text-center">Avg Complexity</th>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {result.metrics.map((m, i) => (
                      <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center text-purple-400">{m.boundaryAwarenessScore}</td>
                        <td className="px-4 py-3 text-center text-red-400">{m.overreachRate}%</td>
                        <td className="px-4 py-3 text-center text-orange-400">{m.underreachRate}%</td>
                        <td className="px-4 py-3 text-center text-green-400">{m.appropriateRoutingRate}%</td>
                        <td className="px-4 py-3 text-center text-gray-400">{m.avgTaskComplexityHandled}ms</td>
