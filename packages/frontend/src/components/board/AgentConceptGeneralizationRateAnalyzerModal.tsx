...
import { AgentConceptGeneralizationRateAnalyzerReport } from '../../api/mutations';

interface Props {
  result: AgentConceptGeneralizationRateAnalyzerReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export function AgentConceptGeneralizationRateAnalyzerModal({ result, loading, onClose }: Props) {
  return (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing concept generalization rate...</span>
              </div>
            </div>
          ) : !result || result.metrics.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <p className="text-gray-400 text-sm">No generalization data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-emerald-900/20 border border-emerald-500/30 rounded-lg px-4 py-3">
                  <p className="text-emerald-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Score</p>
                  <p className="text-emerald-200 text-2xl font-bold">{result.fleetAvgGeneralizationScore}</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Low Generalization Agents</p>
                  <p className="text-red-200 text-2xl font-bold">{result.lowGeneralizationAgents}</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Cross-Domain%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Transfer%</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Adapt Speed</th>
                      <th className="text-left text-gray-400 font-medium pb-2 pr-4">Novel%</th>
                  ...
                  <tbody>
                    {result.metrics.map((m) => (
                      <tr key={m.agentId} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-emerald-400">{m.generalizationScore}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.crossDomainSuccessRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.knowledgeTransferRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.domainAdaptationSpeed}</td>
                        <td className="py-3 pr-4 text-gray-300">{m.novelTaskHandlingRate}%</td>
                        <td className="py-3 pr-4 text-gray-300">{m.totalSessions}</td>
                        <td className="py-3 pr-4">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${trendBadge(m.generalizationTrend)}`}>
                            {m.generalizationTrend}
                          </span>
                        </td>
                        <td className="py-3">
