import type { AgentToolUsageEfficiencyReport } from '../../api/mutations.js';

interface Props {
  result: AgentToolUsageEfficiencyReport | null;
  loading: boolean;
  onClose: () => void;
}
...
export default function AgentToolUsageEfficiencyAnalyzerModal({ result, loading, onClose }: Props) {
  return (
    <div
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">Analyzing tool usage efficiency...</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-gray-400 text-sm">No tool usage data found.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-violet-900/20 border border-violet-500/30 rounded-lg px-4 py-3">
                  <p className="text-violet-400 text-xs font-medium uppercase tracking-wide mb-1">Fleet Avg Efficiency</p>
                  <p className="text-violet-200 text-xl font-bold">{result.fleetAvgEfficiencyScore}%</p>
                </div>
                <div className="bg-red-900/20 border border-red-500/30 rounded-lg px-4 py-3">
                  <p className="text-red-400 text-xs font-medium uppercase tracking-wide mb-1">Inefficient Agents</p>
                  <p className="text-red-200 text-xl font-bold">{result.inefficientAgents}</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-800/60 text-gray-400 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-3 text-center">Redundant</th>
                      <th className="px-4 py-3 text-center">Most Used</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-center">
                            <div className="w-16 bg-gray-700 rounded-full h-1.5">
                              <div className="h-1.5 rounded-full bg-violet-500" style={{ width: `${m.efficiencyScore}%` }} />
                            </div>
                            <span className="text-gray-200 font-mono text-xs">{m.efficiencyScore}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center text-gray-300">{m.totalCalls}</td>
                        <td className="px-4 py-3 text-center text-green-400">{m.successfulCalls}</td>
                        <td className="px-4 py-3 text-center text-red-400">{m.failedCalls}</td>
                        <td className="px-4 py-3 text-center text-yellow-400">{m.redundantCalls}</td>
                        <td className="px-4 py-3 text-center text-gray-300 font-mono text-xs">{m.mostUsedTool}</td>
