...
import { analyzeAgentSpecializationDrift } from '../services/agent-specialization-drift-service.js';
...
export async function agentSpecializationDriftRoutes(fastify: FastifyInstance) {
  fastify.post('/api/projects/:projectId/agent-specialization-drift', async (request, reply) => {
    ...
    const result = await analyzeAgentSpecializationDrift(projectId);
    return result;
  });
}
